 ...
 *
 * Example:
 * ["bob","my_C130"] call cav_aircrafts_fnc_handleLineJump
 * ["bob","my_C130", "NonSteerable_Parachute_F", false] call cav_aircrafts_fnc_handleLineJump
 *
 */
...
private _offset = if (_allowStaggared) then {
    if (GETVAR(_vehicle,GVAR(StaggaredSide),true)) then {
        12+(random [-5,0,5]);
    } else {
        -12+(random [-5,0,5]);
    };
...
if (_allowStaggared) then {
    private _side = GETVAR(_vehicle,GVAR(StaggaredSide),true);
    SETPVAR(_vehicle,GVAR(StaggaredSide),!_side);
};
...
[_player] call FUNC(para,equipment);
